...
DEBUG_GET_ONCE_NUM_OPTION(solarxr_sync_delay_ms, "SOLARXR_SYNC_DELAY_MS", 2)
...
solarxr_device_sync(struct solarxr_device *const device)
{
	...
	uint8_t message[0x200];

	// In testing SlimeVR takes about 0.3ms to respond to poll packets
	assert(device->poll_packet_len < sizeof(message));
	memcpy(message, device->poll_packet, device->poll_packet_len); // `WebSocket_send()` clobbers input data
	WebSocket_send(&device->socket, message, device->poll_packet_len);
	atomic_store(&device->next_sync, time + debug_get_num_option_solarxr_sync_timeout_ms() * U_TIME_1MS_IN_NS);

	// for some reason latency spikes to upwards of 80ms when this is sent before the poll packet
	// clang-format off
	unsigned message_len = (unsigned)snprintf((char*)message, sizeof(message), "%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s", "{"
		"\"type\":\"pos\","
		"\"tracker_id\":0,"
		"\"x\":", (double)head.pose.position.x, ","
		"\"y\":", (double)head.pose.position.y - .2f, "," // SlimeVR adds a hardcoded 0.2 to the WebSocket HMD's Y position
	...
	"}");
	// clang-format on
	assert(message_len < sizeof(message));
	WebSocket_sendWithOpcode(&device->socket, message, message_len, 0x1);
}
...
WebSocket_receive_bundle(struct WebSocket *const socket, const bool exit_on_error)
{
	do {
		const size_t buffer_len = WebSocket_receive(socket);
		if (buffer_len == 0) {
			continue;
		}
	...
	} while (WebSocket_wait(socket));
	if (exit_on_error) {
		U_LOG_IFL_E(socket->log_level, "WebSocket_receive() failed");
	}
	return NULL;
}
...
solarxr_network_thread(void *const ptr)
{
	...
	for (solarxr_protocol_MessageBundle_table_t bundle;
	     (bundle = WebSocket_receive_bundle(&device->socket, false)) != NULL;) {
...
solarxr_device_create_xdevs(struct xrt_device *const hmd, struct xrt_device **const out_xdevs, uint32_t out_xdevs_cap)
{
	...
	if (out_xdevs_cap >= 2) {
		...
		result = WebSocket_send(&device->socket, packet, (uint32_t)packet_len);
		if (!result) {
		...
		const solarxr_protocol_MessageBundle_table_t bundle = WebSocket_receive_bundle(&device->socket, true);
		if (bundle == NULL) {
			solarxr_device_destroy(&device->base);
			return 0;
		}
