...
struct PacketHeader
{
	uint8_t opcode : 4, rsv3 : 1, rsv2 : 1, rsv1 : 1, fin : 1, len : 7, mask : 1,
	    optional[sizeof(uint64_t) + sizeof(uint32_t)];
};
...
bool
WebSocket_sendWithOpcode(struct WebSocket *const state,
                         uint8_t packet[],
                         ...
                         const uint8_t opcode)
{
	...
	if (packet_len < 126) {
		...
		payloadLength_len = 0;
	} else if (packet_len <= UINT16_MAX) {
		header.len = 126;
		payloadLength_len = sizeof(uint16_t);
		memcpy(header.optional, &(const uint16_t){htons(packet_len)}, sizeof(uint16_t));
	} else {
		header.len = 127;
		payloadLength_len = sizeof(uint64_t);
		memcpy(&header.optional[sizeof(uint64_t) - sizeof(uint32_t)], &(const uint32_t){htonl(packet_len)},
		       sizeof(uint32_t));
	}
		...
		if (state->head == state->end && state->finished) {
			...
			case 0x9:
				if (!WebSocket_sendWithOpcode(state, state->buffer,
				                              (uint32_t)(state->head - state->buffer), 0xa)) {
