...
bool
WebSocket_sendWithOpcode(struct WebSocket *state, uint8_t packet[], uint32_t packet_len, uint8_t opcode); // threadsafe
size_t
WebSocket_receive(struct WebSocket *state);

static inline bool
WebSocket_send(struct WebSocket *const state, uint8_t packet[], const uint32_t packet_len)
{
